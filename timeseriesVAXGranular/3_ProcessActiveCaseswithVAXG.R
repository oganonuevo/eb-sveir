...
compileActiveCases <- function(){
    USER = "rserver"
    PASS = "Rpa$$w0rd4321"
    HOST = "fassster.ehealth.ph"
    DB = "fassster"
    COLLECTION = "active_cases"
    URI = sprintf("mongodb://%s:%s@%s/%s", USER, PASS, HOST,DB)
...
writeActiveCases <- function(){
  activedb_changelog <- file_info("./inputs/ActiveCases.rds")
  if ( difftime(Sys.time(), activedb_changelog$modification_time, units = "days") >= 1 ){
    active_cases.df <- readActiveCases()
    #saveRDS(active_cases.df, file = "./inputs/ActiveCases.rds")
  } 
  return(readActiveCases())
}

readActiveCases <- function(){
  active_cases.df <- readRDS("./inputs/ActiveCases.rds") 
  return(active_cases.df)
}


getActiveCases = function(psgcInput, fitParamToUse, filterPSGCs) {
  psgc <- psgcInput %>% as.numeric()
  
  hucPSGC <- read.csv("./inputs/huc_icc.csv") %>%
