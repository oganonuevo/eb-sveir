...
library(stats)
library(fs)
summaries = read.csv("./inputs/parametersVax_20211020.csv") #the file with the summaries
...
if(fitParamToUse == "Cumulative"){
  ...
  source("3_ProcessActiveCaseswithVAX.R")
}else{
...
RunProjections = function(lambdaDateInput, lambdaValuesInput,
  ...
  HcLevel = as.numeric(HcLevel) #deltaS .20 (percentage %)
  
  #vaccination variables
  # vaxDateInput = as.Date(strsplit(vaxDate, ";")[[1]])
  ...
  Covid19Model = function(t, state, parameters){
    with(as.list(c(state,parameters)),{
      ...
      #vax per month or date specified same with lambdaValues    
      # if (t <= tail(vaxequar.l, n = 1)) {
      #   tv1 = (vax1ValuesInput[min(which(t <= vaxequar.l))])
      #   tv2 = (vax2ValuesInput[min(which(t <= vaxequar.l))])
      #   # J = (vaxJValuesInput[min(which(t <= vaxequar.l))])
      # 
      # } else {
      #   tv1 = 0
      #   tv2 = 0
  ...
  # vaxequar.l = as.numeric((vaxDateInput - first_date + 1))
...
# ##### TEST RUN -------------
# if (TRUE){
# HcLevel = "0.2" # removed feature 2020-12-16
...
# lambdaValuesInput = "0.112329449996401;0.0493532397569424"
# projectionDuration = "100"
# psgcInput = "130000000"
# scaleLevel = "0"
# startHcDate = "2021-08-01" # removed feature 2020-12-16
# vax1 = "36537;117181" #37754,45833 65574,45833
# vax2 = "67602;32962"
# jvax = "5905;4275"
# 
