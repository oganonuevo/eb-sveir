...
#* @param v1Value vaccination population (xx)
#* @param v2Value vaccination population (xx)
#* @param jValue vaccination population (xx)
...
function(lambdaDateInput, lambdaValuesInput, psgcInput, 
         ...
         interventionCompliance, dateAdjustment,v1Value,v2Value,jValue) {
  tryCatch({
...
function(psgcInput) {
  ...
  data.df <- read.csv("./inputs/econ_params.csv") %>% filter(area == psgcInput)
